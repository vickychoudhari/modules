...
class MoAuthCustomerSetup extends FormBase
{
  public function getFormId() {
    ...
    public function buildForm(array $form, FormStateInterface $form_state) {
        ...
        elseif ( $current_status == 'PLUGIN_CONFIGURATION') {
            ...
            $mo_table_content = array (
                ...
                array( 'API Key', $mo_db_values['mo_auth_customer_api_key'] ),
                array( 'Token Key', $mo_db_values['mo_auth_customer_token_key'] ),
                array( 'App Secret', $mo_db_values['mo_auth_customer_app_secret'] ),
            );
    ...
    public function validateForm(array &$form, FormStateInterface $form_state) {
        ...
        if( isset( $form_values['Mo_auth_customer_register_username'] ) && !\Drupal::service('email.validator')->isValid( $form_values['Mo_auth_customer_register_username'] )
    ...
    public function submitForm(array &$form, FormStateInterface $form_state) {
        ...
        if ( is_object( $check_customer_response ) && $check_customer_response->status == 'CUSTOMER_NOT_FOUND' ) {
            if ( $tab == 'login' ) {
                \Drupal::messenger()->addError(t('The account with username<b>@username</b> does not exist.'),array('@username'=>$username));
                return;
            }
            // Create customer.
            ...
            if ( json_last_error() == JSON_ERROR_NONE ) {
              ...
              \Drupal::messenger()->addError(t('Something went wrong, Please try again.').' <a href="'. $check_loggers . ' " target="_blank">' .t('Click here'). '</a>'.t('for more details.'));
              return;
        }
    }
